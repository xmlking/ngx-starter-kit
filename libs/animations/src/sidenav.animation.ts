...
import { SidenavState } from '@nx-starter-kit/navigator';
...
export const sidenavAnimation = trigger('sidenavAnimation', [
  ...
  state(
    SidenavState.Mobile,
    style({ position: 'absolute', width: '270px', transform: 'translate3d(-270px, 0, 0)', visibility: 'hidden' })
  ),
  state(
    SidenavState.MobileOpen,
    style({ position: 'absolute', width: '270px', transform: 'translate3d(0, 0, 0)', visibility: 'visible' })
  ),
